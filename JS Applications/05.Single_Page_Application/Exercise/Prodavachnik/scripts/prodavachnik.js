function startApp() {
  ...
  const kinveyAppKey = "kid_r1WZ3J4oG";
  const kinveyAppSecret = "f8b7e3cd88da447096fbcde557c94d70";
  ...
  function listAdverts() {
    ...
    const kinveyAdvertsUrl = kinveyBaseUrl + "appdata/" + kinveyAppKey + "/prodavachnikAdvs";
    const kinveyAuthHeaders = {
    ...
    $.ajax({
      ...
      .then(loadAdvertsSuccess)
      .catch(handleAjaxError)

    function loadAdvertsSuccess(prodavachnikAdvs) {
      showInfo('Advertisements loaded.');
      if (prodavachnikAdvs.length === 0) {
        ...
        for (let advert of prodavachnikAdvs) {
  ...
  function deleteAdvert(advertId) {
    const kinveyBookUrl = kinveyBaseUrl + "appdata/" +
      kinveyAppKey + "/prodavachnikAdvs/" + advertId;
    const kinveyAuthHeaders = {
      'Authorization': "Kinvey " + sessionStorage.getItem('authToken'),
    };
  ...
  function displayAdvert(advertId) {
    ...
    function displayAdvertSuccess(advert) {
      ...
      let advertInfo = $('<div>').append(
        ...
        $('<img>').attr('src', advert.image),
        $('<br>'),
        $('<button class="buttonCreateAd" type="button">Back</button>').on('click', listAdverts));
  ...
  function editAdvert() {
    const kinveyAdvertUrl = kinveyBaseUrl + "appdata/" + kinveyAppKey +
      "/prodavachnikAdvs/" + $('#formEditAd input[name=id]').val();
    const kinveyAuthHeaders = {
      'Authorization': "Kinvey " + sessionStorage.getItem('authToken'),
    };
