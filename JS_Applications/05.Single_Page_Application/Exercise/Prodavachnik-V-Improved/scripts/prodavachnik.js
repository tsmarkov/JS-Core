function startApp() {
  ...
  let requester = (() => {
    ...
    const appKey = 'kid_r1WZ3J4oG';
    const appSecret = 'f8b7e3cd88da447096fbcde557c94d70';
  ...
  async function loadAds() {
    let data = await requester.get('appdata', 'prodavachnikAdvs');
    adsDiv.empty();
    if (data.length === 0) {
  ...
  async function deleteAd(id) {
    await requester.remove('appdata', 'prodavachnikAdvs/' + id);
    showInfo('Ad deleted');
    showView('ads');
  }
  ...
  async function editAd() {
    ...
    try {
      await requester.update('appdata', 'prodavachnikAdvs/' + id, editedAd);
      showInfo('Ad editted');
      showView('ads');
    } catch (err) {
      handleError(err);
  ...
  async function createAd() {
    ...
    try {
      await requester.post('appdata', 'prodavachnikAdvs', newAd);
      showInfo('Ad created');
      showView('ads');
    } catch (err) {
      handleError(err);
